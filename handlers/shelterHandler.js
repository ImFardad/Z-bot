      ...
      await bot.editMessageText(text, {
        ...
        reply_markup: {
          inline_keyboard: [
            [
              {
                text: '🚪 خروج از پناهگاه',
                callback_data: 'shelter_leave_confirm',
              },
            ],
            [
              {
...
async function handleLeaveShelterConfirm(bot, callbackQuery) {
  ...
  try {
    ...
    if (!user || !user.Shelter) {
      console.error(
        `User ${userId} tried to leave a shelter but is not in one.`
      );
      return;
    }
...
async function handleLeaveShelterDo(bot, callbackQuery) {
  ...
  try {
    ...
    if (!user || !user.shelterId) {
      ...
      console.error(
        `User ${userId} tried to execute leave action but was not in a shelter.`
      );
      return;
    }
    ...
    try {
      ...
      await bot.sendMessage(oldShelterId, notificationText, {
        parse_mode: 'Markdown',
      });
    } catch (e) {
      console.error(
        `Failed to send leave notification to shelter ${oldShelterId}:`,
        e.message
      );
    }
