  ...
  try {
    ...
    if (userZombieState[chatId] && userZombieState[chatId].scenario) {
      ...
      await bot.sendMessage(
        chatId,
        '⚠️ **سناریوی فعال**\n\nشما یک سناریوی پاسخ‌داده‌نشده دارید. لطفاً ابتدا راه حل خود را برای آن ارسال کنید.',
        { parse_mode: 'Markdown' }
      );
    ...
    await bot.editMessageText(
      '⏳ در حال تولید سناریوی جدید... لطفاً کمی صبر کنید.',
      {
        chat_id: chatId,
        message_id: messageId,
      }
...
async function handleZombieSolution(bot, msg) {
  ...
  try {
    ...
    await bot.sendMessage(
      chatId,
      '🧠 در حال تحلیل راه حل شما... این فرآیند ممکن است کمی طول بکشد.'
    );

    const evaluation = await geminiService.evaluateZombieSolution(
      scenario,
      userAnswer
    );
    ...
    if (evaluation && typeof evaluation === 'object') {
      try {
        ...
        if (user) {
          ...
          const newSurvivalPercentage =
            (oldSurvivalPercentage + scenarioAverage) / 2;
