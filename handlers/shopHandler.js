...
async function handleShopMenu(bot, callbackQuery) {
  ...
  try {
    ...
    if (user.backpackLevel === 0) {
      ...
      const backpackLevel1 = allItems.find(
        (i) => i.type === 'backpack' && i.level === 1
      );
      if (backpackLevel1) {
        text += `**${backpackLevel1.name}**\n*${backpackLevel1.description}*\n- قیمت: ${backpackLevel1.price} سکه\n`;
        keyboard.push([
          {
            text: `خرید ${backpackLevel1.name}`,
            callback_data: `shop:buy:${backpackLevel1.id}`,
          },
        ]);
      } else {
      ...
      text += '\n**🎒 ارتقای کوله پشتی**\n';
      const backpackLevel2 = allItems.find(
        (i) => i.type === 'backpack_upgrade' && i.level === 2
      );
      const backpackLevel3 = allItems.find(
        (i) => i.type === 'backpack_upgrade' && i.level === 3
      );
        ...
        keyboard.push([
          {
            ...
            callback_data: `shop:buy:${backpackLevel2.id}`,
          },
        ]);
        ...
        keyboard.push([
          {
            ...
            callback_data: `shop:buy:${backpackLevel3.id}`,
          },
        ]);
      } else {
      ...
      for (const item of dailyItems) {
        const purchaseHistory = todayPurchases.filter(
          (p) => p.itemType === item.type
        );
        ...
        if (remaining > 0) {
          ...
          keyboard.push([
            {
              text: `خرید ۱ ${unit} ${item.name}`,
              callback_data: `shop:buy:${item.id}`,
            },
          ]);
        } else {
...
async function handleShopBuyCallback(bot, callbackQuery) {
  ...
  try {
    ...
    if (item.type.startsWith('daily_')) {
      ...
      const totalPurchased = todayPurchases.reduce(
        (acc, p) => acc + p.quantity,
        0
      );
      ...
      if (totalPurchased >= limit) {
        return bot.answerCallbackQuery(callbackQuery.id, {
          text: '⚠️ سهمیه امروز شما برای این آیتم تمام شده است.',
          show_alert: true,
        });
      }
      ...
      if (item.type === 'daily_fuel') {
        if (user.fuel + purchaseQuantity > 5) {
          return bot.answerCallbackQuery(callbackQuery.id, {
            text: '⛽️ ظرفیت حمل سوخت شما کافی نیست.',
            show_alert: true,
          });
        }
      } else if (capacityCost > 0) {
        const backpackCapacity = 50 + (user.backpackLevel - 1) * 25;
        const backpackContent = user.backpackContent
          ? JSON.parse(user.backpackContent)
          : [];
        const usedSpace = backpackContent.reduce(
          (acc, i) => acc + i.quantity * (itemWeights[i.type] || 1),
          0
        );
        if (usedSpace + capacityCost > backpackCapacity) {
          return bot.answerCallbackQuery(callbackQuery.id, {
            text: '🎒 ظرفیت کوله پشتی شما کافی نیست.',
            show_alert: true,
          });
        }
      }
